...
def main() -> None:
    ...
    dataset = LatentDataset(
        ...
        apply_elastic=config['training'].get('use_elastic_deform', False),
        elastic_alpha=config['training'].get('elastic_alpha', 1.0),
        elastic_sigma=config['training'].get('elastic_sigma', 3),
        rescale_raw=True,
        pin_memory=True,
    )
