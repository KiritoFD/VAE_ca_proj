...
class LGTTrainer:
    ...
    def compute_energy_loss(self, x_src, style_id_src, style_id_tgt, epoch, real_style_latents):
        ...
        3. Integrate to x_1
        4. Use real style reference x_style from target distribution
        ...
        # Use real style latents from target distribution (CRITICAL FIX)
        x_style_ref = real_style_latents
        ...
        # Apply per-style dynamic weighting (LGT++ enhancement)
        # Multiply style loss by style-specific weight
        # This applies greater pressure to difficult "quenching" tasks (→photo)
        style_weight_batch = self.style_weights[style_id_tgt].mean()  # Average across batch
        loss_dict['total'] = (
            style_weight_batch * self.energy_loss.w_style * loss_dict['style_swd']
            + self.energy_loss.w_content * loss_dict['content_ssm']
        )
    ...
    def train_epoch(self, dataloader, epoch):
        ...
        for step_idx, batch in enumerate(pbar, start=1):
            ...
            # Sample real style latents from target distribution (CRITICAL FIX)
