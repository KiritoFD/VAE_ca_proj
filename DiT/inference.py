...
def teleport_latent(content_latent, target_style_id, steps, model, device, noise_strength=1.0):
    ...
    Args:
        ...
        steps: int - 积分步数（推荐10-50）
        model: 训练好的DiTModel
        device: cuda/cpu
        noise_strength: float - 噪声强度（1.0=完全噪声，0.0=完全保留原图）
    ...
    # 1. 起点：纯噪声（或部分噪声 for SDEdit效果）
    # noise_strength=1.0: 完全从噪声生成（最强风格化）
    # noise_strength=0.3: 保留更多原图细节
    noise = torch.randn_like(content_latent)
    x_t = content_latent * (1 - noise_strength) + noise * noise_strength
    
    # 2. 结构条件：用户的照片（ControlNet式控制）
...
def image_to_latent(image_path, vae, device):
    ...
    img = img.resize((512, 512))  # SD1.5标准尺寸
    ...
    with torch.no_grad():
        ...
        latent = latent * 0.18215  # SD1.5的scaling factor
...
def find_checkpoint(checkpoint_dir, prefer="best"):
    """
    Automatically find a checkpoint to load
...
def load_model_from_checkpoint(checkpoint_path, model, device):
    """
    Load model from checkpoint (handles both full checkpoint and state_dict only)
    """
...
def main():
    ...
    TARGET_STYLE_ID = 1  # 0 到 num_styles-1
    ...
    input_copy_path = subdir / "input.jpg"
    
    # 保存原始输入图片
    Image.open(INPUT_IMAGE).save(input_copy_path)
    
    print(f"Transferring to style {TARGET_STYLE_ID} with {STEPS} steps (noise={NOISE_STRENGTH})...")
